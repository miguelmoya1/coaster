      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex justify-end mt-4 gap-2">
          <button
            ...
            (click)="canceled.emit()"
          >
            {{ 'roster.create_shift.cancel_btn' | translate }}
          </button>

          <button coaster-btn class="w-full" type="submit" variant="primary" [disabled]="disabled()">
            {{ 'roster.create_shift.submit_btn' | translate }}
          </button>
        </div>
      </div>
    </form>
...
export class CreateShiftForm {
  public readonly members = input.required<BarMember[]>();

  public readonly createShift = output<CreateShiftDto>();
