...
export class CategoriesService {
  ...
  async createCategory(barId: BarId, createCategoryDto: CreateCategoryDto) {
    const category = await this._categoriesRepository.create(
      barId,
      createCategoryDto,
    );
  ...
  async updateCategory(
    ...
    const category = await this._categoriesRepository.update(
      barId,
      categoryId,
      dtos,
    );
  ...
  async deleteCategory(barId: BarId, categoryId: string) {
    ...
    this._barGateway.server
      .to(barId)
      .emit(SocketEvents.CATEGORY_DELETED, { id: categoryId });
  }
}
