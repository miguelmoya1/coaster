...
describe('RegisterPrinterIpHandler', () => {
  ...
  const command = (deviceKey: string, port?: number) =>
    new RegisterPrinterIpCommand('establishment-1' as any, '192.168.1.100', deviceKey, port);
  ...
  it('should throw ForbiddenException with PRINTER_INVALID_DEVICE_KEY if key does not match', async () => {
    readRepo.findByEstablishmentId.mockResolvedValue({ establishmentId: 'establishment-1', deviceKey: 'correct-key' });
  ...
  it('should persist the address when the device key matches', async () => {
    ...
    expect(writeRepo.upsertPrinterConfig).toHaveBeenCalledWith('establishment-1', '192.168.1.100', undefined);
  });
  ...
  it('should persist the port the bridge reports', async () => {
    ...
    expect(writeRepo.upsertPrinterConfig).toHaveBeenCalledWith('establishment-1', '192.168.1.100', 9090);
  });
});
