...
@CommandHandler(ImportTemplatesToEstablishmentCommand)
export class ImportTemplatesToEstablishmentHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: ImportTemplatesToEstablishmentCommand): Promise<void> {
    ...
    const existingCategories = await this.readRepo.findCategoriesByEstablishmentIdAndNames(
    ...
    const createdCategories = await this.readRepo.findCategoriesByEstablishmentIdAndNames(
      command.establishmentId,
      categoryNames,
    );
