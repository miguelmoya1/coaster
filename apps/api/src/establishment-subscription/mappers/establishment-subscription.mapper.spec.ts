...
import { EstablishmentSubscriptionMapper } from './establishment-subscription.mapper';

const NOW = new Date('2026-01-15T00:00:00.000Z');

const buildDbSub = (overrides: Partial<DbEstablishmentSubscription> = {}): DbEstablishmentSubscription =>
  ({
    id: 'sub_id_1',
    ...
    updatedAt: NOW,
    ...overrides,
  }) as DbEstablishmentSubscription;

describe('EstablishmentSubscriptionMapper', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it('should map DbEstablishmentSubscription to domain EstablishmentSubscription with formatted dates', () => {
    const dbSub = buildDbSub();

    const domain = EstablishmentSubscriptionMapper.toDomain(dbSub);
  ...
  describe('effective status', () => {
    it('should downgrade ACTIVE to INACTIVE when no Stripe subscription is linked', () => {
      const domain = EstablishmentSubscriptionMapper.toDomain(buildDbSub({ stripeSubscriptionId: null }));
    ...
    it('should downgrade ACTIVE to INACTIVE when there is no billing period', () => {
      const domain = EstablishmentSubscriptionMapper.toDomain(buildDbSub({ currentPeriodEnd: null }));

      expect(domain.status).toBe(SubscriptionStatus.INACTIVE);
    });

    it('should report ACTIVE as EXPIRED once the period has lapsed', () => {
      const domain = EstablishmentSubscriptionMapper.toDomain(
    ...
    it('should report TRIALING as EXPIRED once the trial has ended', () => {
      const domain = EstablishmentSubscriptionMapper.toDomain(
        buildDbSub({
          status: SubscriptionStatus.TRIALING as DbEstablishmentSubscription['status'],
    ...
    it('should keep CANCELED while the paid period has not lapsed yet', () => {
      const domain = EstablishmentSubscriptionMapper.toDomain(
        buildDbSub({ status: SubscriptionStatus.CANCELED as DbEstablishmentSubscription['status'] }),
      );
  ...
  describe('toFreeDefault', () => {
    it('should build a locked FREE subscription for an establishment that has never subscribed', () => {
      const domain = EstablishmentSubscriptionMapper.toFreeDefault(asEstablishmentId('establishment_id_1'));
  ...
  describe('manual grants', () => {
    ...
    it('should never leak the admin note or the grantor to the workspace payload', () => {
      ...
      const serialised = JSON.stringify(
        EstablishmentSubscriptionMapper.toDomain({ ...dbSub, manualGrantedByName: 'Miguel' }),
      );
    ...
    it('should give the backoffice the reason and the grantor', () => {
      ...
      const domain = EstablishmentSubscriptionMapper.toAdminDomain({ ...dbSub, manualGrantedByName: 'Miguel' });
