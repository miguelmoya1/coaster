...
describe('HandleInvoicePaidHandler (establishment-subscription)', () => {
  ...
  it('should recover a PAST_DUE subscription back to ACTIVE', async () => {
    ...
    expect(writeRepoMock.update).toHaveBeenCalledWith('establishment_123', { status: 'ACTIVE' });
  ...
  it('should recover an UNPAID subscription looked up by customerId when subscriptionId is absent', async () => {
    ...
    readRepoMock.findByStripeCustomerId.mockResolvedValue({ establishmentId: 'establishment_123', status: 'UNPAID' });
