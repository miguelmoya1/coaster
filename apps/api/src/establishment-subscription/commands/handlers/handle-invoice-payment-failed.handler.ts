...
export class HandleInvoicePaymentFailedHandler implements ICommandHandler<HandleInvoicePaymentFailedCommand, void> {
  ...
  async execute(command: HandleInvoicePaymentFailedCommand): Promise<void> {
    ...
    if (!existing) {
      this._logger.debug(
        `Invoice payment failed event ignored: no EstablishmentSubscription found for customerId=${stripeCustomerId}, subscriptionId=${stripeSubscriptionId}`,
      );
      return;
    }

    this._logger.debug(`Marking subscription past due for establishmentId=${existing.establishmentId}`);
    await this._writeRepo.update(existing.establishmentId as EstablishmentId, {
