...
describe('HandleSubscriptionChangedHandler (establishment-subscription)', () => {
  ...
  it('should ignore events for a subscription this establishment does not track', async () => {
    readRepoMock.findByStripeSubscriptionId.mockResolvedValue(null);
    readRepoMock.findByStripeCustomerId.mockResolvedValue({
  ...
  it('should acknowledge a subscription that belongs to no establishment instead of making Stripe retry it', async () => {
  ...
  it('should set status ACTIVE and populate period dates for an active subscription', async () => {
    ...
    readRepoMock.findByStripeSubscriptionId.mockResolvedValue({ establishmentId: 'establishment_123' });
  ...
  it('should fall back to subscription.metadata.establishmentId when no local record exists yet', async () => {
    const subscription = {
      id: 'sub_123',
      customer: 'cus_123',
