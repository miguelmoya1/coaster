...
describe('DuplicateSubscriptionDetectedHandler', () => {
  ...
  it('should report the incident at error level with both subscriptions and the establishment', () => {
    handler.handle(
      new DuplicateSubscriptionDetectedEvent(asEstablishmentId('establishment-1'), 'sub_kept', 'sub_cancelled'),
    );

    expect(error).toHaveBeenCalledTimes(1);
    const message = error.mock.calls[0][0] as string;
    expect(message).toContain('establishment-1');
