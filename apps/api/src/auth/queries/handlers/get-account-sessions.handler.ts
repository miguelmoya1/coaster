...
/**
 * Every refresh mints a new row that inherits the family, so a family is one sign-in on one device and
 * the rows behind it are the trail it left. People think in devices, so that is what this answers with.
 */
...
export class GetAccountSessionsHandler implements IQueryHandler<GetAccountSessionsQuery, AccountSession[]> {
  ...
  #summarise(family: LiveSession[], currentSessionId: string | null): AccountSession {
    // The row nobody has rotated yet is the one the device is holding a refresh token for.
