...
describe('OrdersRepository', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [OrdersRepository, { provide: DbService, useValue: mockPrisma }],
    }).compile();
  ...
  describe('findByBarId', () => {
    it('should find orders by bar id with includes', async () => {
      ...
      db.dbOrder.findMany.mockResolvedValue([]);

      const result = await repository.findByBarId(barId);

      expect(db.dbOrder.findMany).toHaveBeenCalledWith({
        where: { barId },
        include: {
  ...
  describe('findById', () => {
    it('should find an order by id with includes', async () => {
      ...
      db.dbOrder.findUnique.mockResolvedValue({ id: orderId });

      const result = await repository.findById(orderId);

      expect(db.dbOrder.findUnique).toHaveBeenCalledWith({
        where: { id: orderId },
        include: {
          items: { include: { product: true }, orderBy: [{ createdAt: 'asc' }, { id: 'asc' }] },
          table: true,
  ...
  describe('findTableById', () => {
    it('should find a table by id', async () => {
      ...
      db.dbTable.findUnique.mockResolvedValue({ id: tableId });
  ...
  describe('findProductsByIds', () => {
    it('should find products by an array of ids', async () => {
      ...
      db.dbProduct.findMany.mockResolvedValue([{ id: 'prod-1' }, { id: 'prod-2' }]);
  ...
  describe('findOrdersByIds', () => {
    it('should find orders by an array of ids with includes', async () => {
      ...
      db.dbOrder.findMany.mockResolvedValue([{ id: 'order-1' }, { id: 'order-2' }]);
