...
describe('RemoveMember', () => {
  ...
  describe('remove', () => {
    it('should delegate to repository and return the result', async () => {
      ...
      const result = await service.execute(establishmentId, asEstablishmentMemberId(memberId));

      expect(memberRepoMock['remove']).toHaveBeenCalledWith(establishmentId, memberId);
