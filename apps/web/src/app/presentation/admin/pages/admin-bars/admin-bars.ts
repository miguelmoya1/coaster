...
import type { BarBillingSource, BarId } from '@coaster/common';
import { BarBillingSource as BillingSource } from '@coaster/common';
...
const BILLING_FILTERS: (BarBillingSource | undefined)[] = [
  undefined,
...
export default class AdminBars {
  ...
  constructor() {
    ...
    if (requested && BILLING_FILTERS.includes(requested as BarBillingSource)) {
      this.#store.setBillingSource(requested as BarBillingSource);
    }
  }
  ...
  protected selectBillingSource(source: BarBillingSource | undefined) {
    this.#store.setBillingSource(source);
  }

  protected filterLabel(source: BarBillingSource | undefined): string {
    return source ? `admin.billing_source.${source.toLowerCase()}` : 'admin.bars.filter_all';
  }
  ...
  protected openBar(barId: BarId) {
    this.#router.navigate(['/admin/bars', barId]);
  }
}
