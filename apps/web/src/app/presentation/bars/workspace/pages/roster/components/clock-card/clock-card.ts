...
@Component({
  ...
  template: `
    <section
      ...
      [attr.aria-label]="'roster.time_tracking.title' | translate"
    >
      <div class="flex items-center gap-3">
        ...
        <div class="ml-auto flex items-center gap-4 text-right">
          ...
          <div class="flex flex-col">
            <span class="text-xxs font-bold uppercase tracking-wider text-on-surface-variant">
              {{ 'roster.time_tracking.break' | translate }}
            </span>
      ...
      <div class="flex flex-wrap gap-2">
        @if (state() === ClockState.OUT) {
          <button mat-flat-button class="flex-1" [disabled]="disabled()" (click)="clock.emit(TimeEntryType.CLOCK_IN)">
            ...
            {{ 'roster.time_tracking.clock_in' | translate }}
          </button>
        }
        ...
        @if (state() === ClockState.IN) {
          ...
          >
            ...
            {{ 'roster.time_tracking.break_start' | translate }}
          </button>
        }

        @if (state() === ClockState.ON_BREAK) {
          <button mat-stroked-button class="flex-1" [disabled]="disabled()" (click)="clock.emit(TimeEntryType.BREAK_END)">
            <mat-icon>play_arrow</mat-icon>
            {{ 'roster.time_tracking.break_end' | translate }}
          </button>
        }
        ...
        @if (state() !== ClockState.OUT) {
          <button mat-flat-button class="flex-1" [disabled]="disabled()" (click)="clock.emit(TimeEntryType.CLOCK_OUT)">
            ...
            {{ 'roster.time_tracking.clock_out' | translate }}
          </button>
        }
      </div>
    </section>
...
export class ClockCard {
  ...
  protected readonly stateLabel = computed(() => `roster.time_tracking.state_${this.state().toLowerCase()}`);
