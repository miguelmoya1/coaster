...
@Component({
  selector: 'coaster-establishment-role-badge',
  imports: [TranslatePipe],
  template: `
    <div data-testid="establishment-role-badge" class="flex items-center gap-2 mt-2">
      <div
        data-testid="establishment-role-badge-dot"
        class="w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] opacity-90"
        [class]="dotColorClass()"
      ></div>
      <span
        data-testid="establishment-role-badge-label"
...
export class EstablishmentRoleBadge {
  public readonly role = input<EstablishmentRole | undefined>(undefined);
  ...
  public readonly labelKey = computed(() => {
    switch (this.role()) {
      ...
      default:
        return 'establishments.select.operational';
    }
  });
}
