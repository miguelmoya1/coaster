...
describe('AiVoiceService', () => {
  ...
  const repositoryMock = {
    ...
    streamCommand: vi
      .fn()
      .mockImplementation(async (_establishmentId, _prompt, _messages, onDelta: (delta: string) => void) => {
  ...
  describe('state transitions', () => {
    ...
    it('should transition through states on send', async () => {
      ...
      service.send(asEstablishmentId('establishment-1'));
    ...
    it('should surface the answer progressively while it streams', async () => {
      repositoryMock.streamCommand.mockImplementationOnce(
        async (_establishmentId: unknown, _prompt: unknown, _messages: unknown, onDelta: (delta: string) => void) => {
          onDelta('Mesa 5 ');
      ...
      service.transcript.set('Crear mesa Mesa 5');
      await service.send(asEstablishmentId('establishment-1'));
      TestBed.flushEffects();
