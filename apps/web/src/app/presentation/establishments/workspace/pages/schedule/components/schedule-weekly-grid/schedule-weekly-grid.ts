...
@Component({
  selector: 'coaster-schedule-weekly-grid',
      ...
      >
        <div class="flex flex-col gap-1 min-w-0">
          <h3 class="font-bold text-white text-sm uppercase tracking-wider flex items-center gap-2">
            ...
            {{ 'schedule.replication.title' | translate }}
          </h3>
          <p class="text-xs text-on-surface-variant leading-relaxed max-w-xl">
            {{ 'schedule.replication.description' | translate }}
          </p>
        </div>
        <button
          mat-flat-button
        ...
        >
          ...
          {{ 'schedule.replication.button' | translate }}
        </button>
      </div>
    }
...
export class ScheduleWeeklyGrid {
