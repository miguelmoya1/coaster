...
export class Dashboard {
  ...
  readonly trialDaysRemaining = computed(() => this.#establishmentSubscriptionStore.trialDaysRemaining());
  readonly isTrialActive = computed(() => this.#establishmentSubscriptionStore.isTrialActive());
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.#statsStore.setEstablishmentId(establishmentId);
    });
  }
    ...
    return shifts
      .map((s) => {
        ...
        return {
          ...
          roleLabel: member.role === EstablishmentRole.OWNER ? 'Owner' : 'Staff Member',
  ...
  readonly overviewStats = computed(() => {
    ...
    return [
      {
        label: 'dashboard.overview.establishment_staff',
        count: members.filter((m) => m.role === EstablishmentRole.STAFF).length,
      },
