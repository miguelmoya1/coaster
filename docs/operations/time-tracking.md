...
> Being moved out to [Fichit](fichit-integration.md), which does this as its whole product. A venue
> with `fichitClockingSince` set clocks there, and this module keeps serving its stretch of the
> history read-only. What follows describes that stretch, and every venue that has not moved yet.
...
Every row carries `sequence` (monotonic per establishment), `prevHash` and `hash`:

```text
hash = sha256(prevHash + "id|establishmentId|userId|rootId|type|action|occurredAt|recordedAt|workdayDate|userName|userEmail|source|supersedesId|actorId|reason|sequence")
```

Inserting takes `pg_advisory_xact_lock(hashtext(establishmentId))` so two simultaneous clock-ins at the same
establishment cannot fork the chain. The `id` is a UUID generated in the repository, not by the database,
...
`GET /establishments/:establishmentId/time-entries/integrity` recomputes the whole chain for an establishment and reports whether
...
belongs entirely to the day it started. The timezone is `Europe/Madrid` (`ESTABLISHMENT_TIME_ZONE`).
...
Everyone corrects their own marks (`establishment:amend-own-time-entry`, which everybody has) with a reason.
Touching somebody else's requires `establishment:manage-time-entries`. There are no approvals and no
...
| ----------------- | ------------------------------------ | -------------------------------------------------------------------------- |
| `POST /clock`     | `establishment:clock-in`             | Clock yourself in or out, and breaks                                       |
| `GET /me`         | membership                           | My workday with its history of changes                                     |
| `GET /`           | `establishment:view-time-entries`    | The team's workdays, filterable by person                                  |
| `GET /export`     | `establishment:view-time-entries`    | CSV, one row per revision, free `from`/`to` range                          |
| `GET /integrity`  | `establishment:manage-time-entries`  | Hash chain verification                                                    |
| `POST /`          | `establishment:manage-time-entries`  | Manually add a forgotten mark                                              |
| `POST /:id/amend` | `establishment:amend-own-time-entry` | Correct a time (own, or anyone's with `establishment:manage-time-entries`) |
| `POST /:id/void`  | `establishment:manage-time-entries`  | Void a mark                                                                |

`establishment:clock-in` and `establishment:amend-own-time-entry` are held by everyone; the rest by `MANAGER` and
...
`TimeEntry`'s foreign keys are `RESTRICT`, not `CASCADE`: deleting a user or an establishment that has marks
...
- **Clock card** (`clock-card`): current state, time worked and on break, and only the buttons the
  day allows at that moment. Visible to anyone with `establishment:clock-in`, which is everyone.
...
- **Team register**: the same card per worker, for anyone with `establishment:view-time-entries`, plus
  **Download CSV**. With `establishment:manage-time-entries` the correct, void, add-mark and verify-integrity
  actions appear too.
...
The shifts page does not look at `EstablishmentRole.OWNER` to decide what to show. Creating shifts, the weekly
replication block and the delete button are governed by `establishment:create-shift` and `establishment:delete-shift`.
