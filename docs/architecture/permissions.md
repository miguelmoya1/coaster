...
An `ADMIN` walks into any establishment with `OWNER` powers without being a member. Three points make that
...
An admin can give an establishment PRO without going through Stripe. It lives in its own columns on
`EstablishmentSubscription`, **separate from the Stripe ones**:
