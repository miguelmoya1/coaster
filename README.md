...
| `does not provide an export named '...'`                                                              | Vite pre-bundles `@coaster/common` into `apps/web/.angular/cache` and reuses that bundle across restarts, so adding or renaming an export leaves the old one in place | `docker compose exec web rm -rf /app/apps/web/.angular/cache && docker compose restart web`. Delete it **from inside the container**: removing the directory from the host while the container holds it open detaches the bind mount, after which the container writes to its own copy and nothing you do on the host has any effect. Then open the page in a new tab — the browser caches the ES module by URL and keeps serving the old one |
